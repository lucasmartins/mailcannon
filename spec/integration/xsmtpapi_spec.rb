...
describe "X-SMTPAPI compatibility" do
  describe "xsmtpapi for bulk", sidekiq: :inline do
    ...
    context "generates expected xsmtpapi for #post!" do
      ...
      let(:envelope) { build(:envelope_multi, xsmtpapi: { "sub" => { "-email-id-" => %w[314159 271828] }, "unique_args" => { "email_id" => "-email-id-" } }) }
      let(:expectated_hash) { { "sub" => { "-email-id-" => %w[314159 271828] }, "to" => ["mailcannon@railsnapraia.com", "lucasmartins@railsnapraia.com", "contact@railsonthebeach.com"], "unique_args" => { "email_id" => "-email-id-", "envelope_id" => envelope.id, "envelope_bag_id" => envelope_bag.id } } }
